...
Verify Page Loader Completed Successfully 
    ...
    Run Keyword If    ${isListLoaderPresent}    Element Should Not Be Visible [Arguments] ${listLoader}
    Element Should Be Visible [Arguments] ${homePageLogo}  
...
Verify Toast Message Disappeared
    ...
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Page Contains Element    ${toastMessage}
...
Element Should Be Contained In The Page [Arguments] ${locator}
    ...
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Page Contains Element    ${locator}
...
Element Should Not Be Contained In The Page [Arguments] ${locator}
    ...
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Page Does Not Contain Element   ${locator}
...
Element Should Be Visible [Arguments] ${locator}
    ...
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Element Is Visible    ${locator}
...
Element Should Not Be Visible [Arguments] ${locator}
    ...
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Element Is Not Visible   ${locator}
...
Click Element [Arguments] ${locator}
    ...
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Element Is Enabled    ${locator}
...
Element Text Should Be [Arguments] ${locator} ${text}
    ...
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Element Is Visible    ${locator}
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Element Text Should Be    ${locator}    ${text}    


Set Text [Arguments] ${textBoxLocator} ${text}
    [Documentation]        
	...  Input text into a text box  ...

    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Element Is Enabled    ${textBoxLocator}
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Input Text    ${textBoxLocator}    ${text}


Select From Dropdown [Arguments] ${dropdownLocator} ${text}
    [Documentation]        
	...  Select an option value from a dropdown  ...

    Click Element [Arguments] ${dropdownLocator}
    ${optionLocator}    Get Option Locator By Name From Dropdown [Arguments] ${text}
    Click Element [Arguments] ${optionLocator}


Click The Checkbox [Arguments] ${checkBoxLabelVal}
    [Documentation]        
    ...
    Click Element [Arguments] ${checkBoxLocator}


Check The Checkbox [Arguments] ${checkBoxLocator}
    [Documentation]        
	...  Check the check box, only if the checkbox status is unchecked  ...
    
    Wait Until Keyword Succeeds    ${RETRY_SCALE}x     1 s    Wait Until Element Is Enabled  ${checkBoxLocator}
	${isCheckboxSelected}    Run Keyword And Return Status    Checkbox Should Be Selected    ${checkBoxLocator}
    Run Keyword If    '${isCheckboxSelected}'== 'False'    Click Element    ${checkBoxLocator}


Select The Date From DatePicker [Arguments] ${datePickerLocator} ${year} ${month} ${date}
    [Documentation]        
	...  Select a date from the date picker  ...
    
    Click Element [Arguments] ${datePickerLocator}
    Click Element [Arguments] ${datePickerCurrentMonthYearElement}
    Click Element [Arguments] ${datePickerCurrentMonthYearElement}
    Click Element [Arguments] ${datePickerPreviousButton}    
    Click Element [Arguments] ${datePickerPreviousButton}    
    Click Element [Arguments] ${datePickerPreviousButton}    
    ${yearLocator}   Get Locator By Text [Arguments] ${year}
    Click Element [Arguments] ${yearLocator}
    ${monthLocator}   Get Locator By Text [Arguments] ${month}
    Click Element [Arguments] ${monthLocator}
    ${dateLocator}   Get Locator By Text [Arguments] ${date}
    Click Element [Arguments] ${dateLocator}